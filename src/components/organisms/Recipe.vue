...
<template>
    <Spinner class="bg-white justify-center items-start" v-if="store.loadingRecipe" />
    <div v-else-if="Object.keys(store.recipe).length > 0 " class="recipe bg-ivory">
        <figure
            ...
            <div 
            @click="state.toggleSidebar(true)"
            :class="{'invisible' : store.sidebarVisible }"
            class="recipe__goback md:invisible h-12 pl-4 pr-8 bg-white text-2xl absolute top-6 left-6 rounded-full flex justify-center items-center cursor-pointer">
                <font-awesome-icon @click="decrease" icon="fa-solid fa-angle-left"
                    class="mr-4 text-textdark h-10 w-10" />
        ...
        <div class="recipe__details flex items-center pt-32 px-32 pb-14">
            ...
            <button
                ...
                <font-awesome-icon @click="state.toggleBookmark(store.recipe)" icon="fa-regular fa-bookmark"
                    class="h-10 w-10" />
            </button>
        </div>
        ...
        <div class="recipe__directions flex flex-col py-20 px-40 items-center">
            ...
            <p class="recipe__directions-text text-[1.7rem] mb-14">
                This recipe was carefully designed and tested by <span
                    class="font-bold">{{store.recipe.publisher}}</span>. Please check out directions at their website.
            </p>
            <RoundedButton iconRight="fa-solid fa-arrow-right" content="Directions" :url="store.recipe.source_url" />
        </div>
    </div>
    <div v-else class="recipe bg-ivory">
        <div class="norecipe max-w-2xl mx-auto flex px-16 py-20">
            ...
            <p class="text-3xl font-bold leading-relaxed">Start by searching for a recipe or an ingredient. Have fun!
            </p>
        </div>
    </div>
</template>